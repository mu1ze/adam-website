...
export default function SpaceInvadersPage() {
  ...
  const startGame = () => {
    ...
    for (let row = 0; row < 5; row++) {
      for (let col = 0; col < 11; col++) {
        newAliens.push({ x: 50 + col * (ALIEN_WIDTH + 15), y: 50 + row * (ALIEN_HEIGHT + 15), row });
      }
    }
    stateRef.current = {
      shipX: GAME_WIDTH / 2 - SHIP_WIDTH / 2,
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (state.keys.ArrowLeft) state.shipX = Math.max(0, state.shipX - 5);
      if (state.keys.ArrowRight) state.shipX = Math.min(GAME_WIDTH - SHIP_WIDTH, state.shipX + 5);
      ...
      if (shouldFire && now - state.lastShot > 400) {
        state.bullets.push({ x: state.shipX + SHIP_WIDTH / 2 - BULLET_WIDTH / 2, y: GAME_HEIGHT - SHIP_HEIGHT - 10 - BULLET_HEIGHT });
        state.lastShot = now;
      }
      ...
      for (let i = state.bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = state.aliens.length - 1; j >= 0; j--) {
          ...
          if (b.x < a.x + ALIEN_WIDTH && b.x + BULLET_WIDTH > a.x && b.y < a.y + ALIEN_HEIGHT && b.y + BULLET_HEIGHT > a.y) {
      ...
      if (state.aliens.length === 0) { handleWin(state.score); return; }
      ...
      if (now - state.lastAlienMove > state.alienSpeed) {
        ...
        state.aliens.forEach(a => {
          if ((state.alienDirection === 1 && a.x + ALIEN_WIDTH + 10 > GAME_WIDTH) ||
              (state.alienDirection === -1 && a.x - 10 < 0)) hitEdge = true;
        });
        if (hitEdge) {
          state.alienDirection *= -1;
          state.aliens.forEach(a => a.y += 20);
          state.alienSpeed = Math.max(20, state.alienSpeed - 2);
      ...
      if (now - state.lastAlienFire > 1000 - (55 - state.aliens.length) * 15 && state.aliens.length > 0) {
        ...
        state.alienBullets.push({ x: randomAlien.x + ALIEN_WIDTH / 2 - BULLET_WIDTH / 2, y: randomAlien.y + ALIEN_HEIGHT });
      }

      for (let i = state.alienBullets.length - 1; i >= 0; i--) {
        const b = state.alienBullets[i];
        b.y += 5;
        if (b.x < state.shipX + SHIP_WIDTH && b.x + BULLET_WIDTH > state.shipX &&
            b.y < GAME_HEIGHT - 10 && b.y + BULLET_HEIGHT > GAME_HEIGHT - 10 - SHIP_HEIGHT) {
          handleGameOver(state.score);
          return;
        }
        if (b.y > GAME_HEIGHT) state.alienBullets.splice(i, 1);
      }
  ...
  const render = () => {
    ...
    state.alienBullets.forEach(b => ctx.fillRect(b.x, b.y, BULLET_WIDTH, BULLET_HEIGHT));
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === ' ' || e.key === 'Spacebar') {
        ...
        if (gameState === 'READY' || gameState === 'GAMEOVER' || gameState === 'WIN') startGame();
        e.preventDefault();
      }
    };
    const handleKeyUp = (e) => {
